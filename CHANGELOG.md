...
- Recovered signed ImageMagick v7.1.2-30 and rewrote the smart WebP/AVIF/HEIC converter. Static images now produce validated JPGs; animations use direct FFmpeg VFR conversion with preserved timing and full-decode validation. Existing outputs and sources are preserved, batch failures return nonzero without stopping valid siblings, the preflight recognizes standard ImageMagick installs, and 25 real assertions pass in PowerShell 7 and 5.1. Registry installation awaits the explicit Limited security-policy choice.
